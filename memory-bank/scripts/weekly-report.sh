...
if [ -n "$1" ]; then
    ...
    WEEK_START=$(date -v "monday" +%Y-%m-%d 2>/dev/null || date -v monday +%Y-%m-%d 2>/dev/null || date +%Y-%m-%d)
fi

WEEK_END=$(date -v "$WEEK_START + 6 days" +%Y-%m-%d 2>/dev/null || date -v +6d -j -f %Y-%m-%d "$WEEK_START" +%Y-%m-%d 2>/dev/null || date +%Y-%m-%d)
WEEK_NUMBER=$(date -v "$WEEK_START" +%V 2>/dev/null || date -j -f %Y-%m-%d "$WEEK_START" +%V 2>/dev/null || echo "$(date +%V)")
YEAR=$(date -v "$WEEK_START" +%Y 2>/dev/null || date -j -f %Y-%m-%d "$WEEK_START" +%Y 2>/dev/null || echo "$(date +%Y)")
...
get_completed_this_week() {
    local week_start_epoch=$(date -v "$WEEK_START" +%s 2>/dev/null || date -j -f %Y-%m-%d "$WEEK_START" +%s 2>/dev/null || echo "0")
    local week_end_epoch=$(date -v "$WEEK_END 23:59:59" +%s 2>/dev/null || date -j -f "%Y-%m-%d %H:%M:%S" "$WEEK_END 23:59:59" +%s 2>/dev/null || echo "9999999999")
